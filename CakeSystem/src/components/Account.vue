<template>
	<div>
		<div class="middle">
			<el-input class="inp" placeholder="请输入您的账号" v-model="acInput" ref="adminname" clearable></el-input>
				...
				if(name != '' && name != ''){
					...
					axios.post("http://localhost:8888/adminLogin",{
						...
						if(response.data.login==1){
							...
							localStorage.setItem("adName",response.data.adName);
							
							// this.$router.push({path:"/"});
							location.reload(this.$router.push({path:"/home"}));
