		...
		<div class="pro-detail-box">
			<div class="left">
				<div class="title-area">
					<div class="p-name"><span>{{list.coff_name}}</span><span class="price">￥{{list.coff_price}}</span></div>
					<div class="p-detail">{{list.coff_name}}</div>
					<a>
					...
					<div class="btn-box">
						<div class="add-btn-box">
							<button class="btn add-btn"
								@click="addToCart($route.query.coffid,$route.query.coffname)">加入购物车+</button>
							<div v-if="isLogin">
						...
						<button class="btn buy-btn" @click="jumpToCart($route.query.coffid,$route.query.coffame)">立即购买</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	export default {
		...
		mounted() {
			...
			this.isAdd = false;
			this.$http.post("http://localhost:8888/showCoffee", {
				coff_id: this.$route.query.coffid
			}).then((response) => {
				// console.log(response.data);
