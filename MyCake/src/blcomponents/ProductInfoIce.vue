		...
		<div class="pro-detail-box">
			<div class="left">
				<div class="title-area">
					<div class="p-name"><span>{{list.ice_name}}</span><span class="price">￥{{list.ice_price}}</span></div>
					<div class="p-detail">{{list.ice_name}}</div>
					<a>
					...
					<div class="btn-box">
						<div class="add-btn-box">
							<button class="btn add-btn"
								@click="addToCart($route.query.iceid,$route.query.icename)">加入购物车+</button>
							<div v-if="isLogin">
						...
						<button class="btn buy-btn" @click="jumpToCart($route.query.iceid,$route.query.icename)">立即购买</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	export default {
		...
		mounted() {
			...
			this.isAdd = false;
			this.$http.post("http://localhost:8888/showIceCream", {
				ice_id: this.$route.query.iceid
			}).then((response) => {
				// console.log(response.data);
