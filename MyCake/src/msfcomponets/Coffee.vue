<template>
	<div class="contain">
		...
		<div class="box">
			<ul>
				<li v-for="item in CoffeeList" :key='item.id' class="item">
					<img v-bind:src="item.coff_img" alt="" @click="jump(item.id,item.coff_name)">
					<h3>{{item.coff_name}}</h3>
					<p class="list">{{item.coff_price}}</p>
					...
					<div :class="{ 'ShowCat': true, 'trafrome': item.isSort }">
						<div class="btn" v-if="item.isSort">
							...
							<span class="clan" @click='btn_addCat(item,item.id,item.coff_name)'>加入购物车</span>
						</div>
					</div>
		...
		methods: {
			getCoffeeList() {
				this.$http.get("http://localhost:8888/coffee")
					.then(response => {
						this.CoffeeList = response.data.map(data => {
							data.isSort = false;
			...
			jump(id,name){
				this.$router.push({ path: "/productinfocoff", query: { coffid: id, coffname: name } });
			}
