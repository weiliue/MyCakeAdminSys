<template>
	<div class="contain">
		...
		<div class="box">
			<ul>
				<li v-for="item in IceCreamList" :key='item.id'>
					<img v-bind:src="item.ice_img" alt="" @click="jump(item.id,item.ice_name)">
					<h3>{{item.ice_name}}</h3>
					<p class="list">{{item.ice_price}}</p>
					...
					<div :class="{ 'ShowCat': true, 'trafrome': item.isSort }">
						<div class="btn" v-if="item.isSort">
							...
							<span class="clan" @click='btn_addCat(item,item.id,item.ice_name)'>加入购物车</span>
						</div>
					</div>
		...
		methods: {
			getIceCreamList() {
				this.$http.get("http://localhost:8888/icecream")
					.then(response => {
						this.IceCreamList = response.data.map(data => {
							data.isSort = false;
			...
			jump(id,name){
				this.$router.push({ path: "/productinfoice", query: { iceid: id, icename: name } });
			}
		},
	}
</script>
