<template>
	<div class="containter">
		<van-nav-bar :title="list.bre_name" left-text="返回" left-arrow @click-left="onClickLeft" @click-right="onClickRight" :fixed="true" />
		<div class="img-box">
			<img :src="list.bre_img" alt="">
		</div>
		<div class="detail-outer-box">
			<div class="detail-box">
				<p class="name">{{list.bre_name}}</p>
				<p class="desc">{{list.bre_infor}}</p>
				<p class="price">{{list.bre_price}}</p>
			</div>
		...
		<van-goods-action>
			...
			<van-goods-action-button type="warning" text="加入购物车" @click="addToCart(list.id,list.bre_name)" />
			<van-goods-action-button type="danger" text="立即购买" @click="jumpToCartPage($route.query.breid,$route.query.brename)" />
		</van-goods-action>
...
<script>
	export default {
		...
		mounted() {
			...
			this.isAdd = false;
			this.$http.post("http://localhost:8888/showBreadInfor", {
				id: this.$route.query.breid
			}).then((response) => {
				// console.log(response.data);
