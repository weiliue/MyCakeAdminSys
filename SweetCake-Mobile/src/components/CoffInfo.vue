<template>
	<div class="containter">
		<van-nav-bar :title="list.coff_name" left-text="返回" left-arrow @click-left="onClickLeft" @click-right="onClickRight" :fixed="true" />
		<div class="img-box">
			<img :src="list.coff_img" alt="">
		</div>
		<div class="detail-outer-box">
			<div class="detail-box">
				<p class="name">{{list.coff_name}}</p>
				<p class="desc">{{list.coff_infor}}</p>
				<p class="price">{{list.coff_price}}</p>
			</div>
		...
		<van-goods-action>
			...
			<van-goods-action-button type="warning" text="加入购物车" @click="addToCart(list.id,list.coff_name)" />
			<van-goods-action-button type="danger" text="立即购买" @click="jumpToCartPage($route.query.coffid,$route.query.coffname)" />
		</van-goods-action>
...
<script>
	export default {
				...
				if (localStorage.getItem("isLogin")) {
					...
					this.$http.get(`http://localhost:8888/queryCartInfor?name=${proName}&&userId=${userId}`) //查询购物车的商品
						.then((response) => {
							...
							if (result.msg == 0) {
								that.$http.get(`http://localhost:8888/insertCoffeeCartInfor?id=${produId}&&userId=${userId}`)
									.then((response) => {
		...
		mounted() {
			...
			this.isAdd = false;
			this.$http.post("http://localhost:8888/showCoffeeInfor", {
				id: this.$route.query.coffid
			}).then((response) => {
				console.log(response.data);
