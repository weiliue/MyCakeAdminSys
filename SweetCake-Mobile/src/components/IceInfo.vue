<template>
	<div class="containter">
		<van-nav-bar :title="list.ice_name" left-text="返回" left-arrow @click-left="onClickLeft" @click-right="onClickRight" :fixed="true" />
		<div class="img-box">
			<img :src="list.ice_img" alt="">
		</div>
		<div class="detail-outer-box">
			<div class="detail-box">
				<p class="name">{{list.ice_name}}</p>
				<p class="desc">{{list.ice_infor}}</p>
				<p class="price">{{list.ice_price}}</p>
			</div>
			<div class="taste">
				<ul>
					<li>
						<img src="https://static.21cake.com/themes/wap/img/goods/icon/micon-31.png" alt="">
						<span>冰淇淋</span>
					</li>
				</ul>
			</div>
		...
		<van-goods-action>
			...
			<van-goods-action-button type="warning" text="加入购物车" @click="addToCart(list.id,list.ice_name)" />
			<van-goods-action-button type="danger" text="立即购买" @click="jumpToCartPage($route.query.iceid,$route.query.icename)" />
		</van-goods-action>
...
<script>
	export default {
				...
				if (localStorage.getItem("isLogin")) {
					...
					this.$http.get(`http://localhost:8888/queryCartInfor?name=${proName}&&userId=${userId}`) //查询购物车的商品
						.then((response) => {
							...
							if (result.msg == 0) {
								that.$http.get(`http://localhost:8888/insertIcecreamCartInfor?id=${produId}&&userId=${userId}`)
									.then((response) => {
		...
		mounted() {
			...
			this.isAdd = false;
			this.$http.post("http://localhost:8888/showCake", {
				id: this.$route.query.iceid
			}).then((response) => {
				// console.log(response.data);
