<template>
	<div>
		<div class="pro-list">
			<div class="pro-item" v-for="item in coffeeList">
				<img :src="item.coff_img" alt="" @click="jumpToIceInfo(item.id,item.coff_name)">
				<p class="pro-name">{{item.coff_name}}</p>
				<div class="pro-bottom">
					<span class="pro-price">{{item.coff_price}}</span>
					<img src="https://static.21cake.com//themes/wap/img/list-cart.png" alt="" @click="addToCart(item.id,item.coff_name,item)">
				</div>
...
<script>
	export default {
		...
		methods:{
			...
			jumpToIceInfo(id,coff_name){
				this.$router.push({path:"/coffinfo",query:{coffid:id,coffname:coff_name}})
			},
		...
		mounted(){
			this.$http.get("http://localhost:8888/coffee")
			.then((res)=>{
				// console.log(res.data);
				this.coffeeList = res.data.map(data=>{
					data.isShow = false;
					return data;
				});
			}).catch(function(error){
