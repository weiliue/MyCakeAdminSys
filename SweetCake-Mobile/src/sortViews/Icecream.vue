<template>
	<div>
		<div class="pro-list">
			<div class="pro-item" v-for="item in iceList">
				<img :src="item.ice_img" alt="" @click="jumpToIceInfo(item.id,item.ice_name)">
				<p class="pro-name">{{item.ice_name}}</p>
				<div class="pro-bottom">
					<span class="pro-price">{{item.ice_price}}</span>
					<img src="https://static.21cake.com//themes/wap/img/list-cart.png" alt="" @click="addToCart(item.id,item.ice_name,item)">
				</div>
...
<script>
	export default {
		...
		methods:{
			...
			jumpToIceInfo(id,ice_name){
				this.$router.push({path:"/iceinfo",query:{iceid:id,icename:ice_name}})
			},
		...
		mounted(){
			this.$http.get("http://localhost:8888/icecream")
			.then((res)=>{
				// console.log(res.data);
				this.iceList = res.data.map(data=>{
					data.isShow = false;
					return data;
				});
			}).catch(function(error){
